...
public enum Authenticator {
    ...
    */
    ...
    case HMAC(key: [UInt8], variant:Gather4_dev.HMAC.Variant)
    ...
    */
    public func authenticate(message: [UInt8]) -> [UInt8]? {
        ...
        case .Poly1305(let key):
            return Gather4_dev.Poly1305.authenticate(key: key, message: message)
        case .HMAC(let key, let variant):
            return Gather4_dev.HMAC.authenticate(key: key, message: message, variant: variant)
        }
    }
}