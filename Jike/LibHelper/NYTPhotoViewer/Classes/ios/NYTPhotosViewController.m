...
- (instancetype)initWithPhotos:(NSArray *)photos initialPhoto:(id <NYTPhoto>)initialPhoto {
    ...
    if (self) {
        ...
        tmpBtn.frame = CGRectMake(0, 0, 28, 28);
        [tmpBtn addTarget:self action:@selector(doneButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
        _overlayView.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:tmpBtn];
...
-(void)photoViewController:(NYTPhotoViewController *)photoViewController tapToWithGestureRecognizer:(UITapGestureRecognizer *)tapGestureRecognizer
{
    [self doneButtonTapped:nil];
}
